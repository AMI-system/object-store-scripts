    ...
    for country in all_countries:
        ...
        for dep in sorted(all_deps):
            ...
            dep_list = {
                    ...
                    "name": dep_info['location_name'], 
                    "deployment_key": f"{dep_info['location_name']} - {dep_info['camera_id']}", 
                    "location_id": dep_info['location_id'], 
                    "latitute": dep_info['lat'], 
                    "longitute": dep_info['lon'], 
                    "camera_id": dep_info['camera_id'],
                    "system_id": dep_info['system_id'],
                    "status": dep_info['status'],
                    "image_count": count
                }
            all_dep_info.append(dep_list)

    # save all data to file
    with open("outputs.json", "w") as final:
        json.dump(all_dep_info, final)
