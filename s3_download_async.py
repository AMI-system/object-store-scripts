...
async def download_object(s3_client, bucket_name, key, download_path,
                      perform_inference=False, remove_image=False, localisation_model=None, binary_model=None, species_model=None, country='UK', region='UKCEH'):
    """
    ...
    try:
        ...
        if perform_inference:
            await perform_inf(download_path, loc_model = localisation_model, binary_model=binary_model, regional_model=species_model, country=country, region=region)
        if remove_image:
...
async def get_objects(bucket_name, key,
                local_path,
                batch_size=100,
        ...
        async for page in page_iterator:
            for obj in page.get("Contents", []):
                ...
                if len(keys) >= batch_size:
                    await download_batch(s3_client, bucket_name, keys, local_path, perform_inference,
...
def display_menu():
    ...
    for region in deps:
        ...
        print(prefix)        # # Run the asynchronous download
        asyncio.run(
